...
ACTION_TO_IDX = {"Stop": 0, "North": 1, "South": 2, "East": 3, "West": 4}
...
def train_model(model, train_loader, test_loader, device, num_epochs=NUM_EPOCHS):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (maps, actions) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 10 == 0:
                print(
                    f"Epoch: {epoch+1}/{num_epochs}, Batch: {batch_idx+1}/{len(train_loader)}, Loss: {train_loss/(batch_idx+1):.4f}, Acc: {100.*train_correct/train_total:.2f}%"
                )
        ...
        print(
            f"Epoch: {epoch+1}/{num_epochs}, Train Loss: {train_loss/len(train_loader):.4f}, Test Loss: {test_loss/len(test_loader):.4f}, Test Acc: {test_accuracy:.2f}%"
        )
